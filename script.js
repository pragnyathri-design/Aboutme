...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (qf && Array.isArray(c.quick_facts)) {
  ...
  if (mailLink) {
    ...
    mailLink.addEventListener("click", (e) => {
      // Always copy email to clipboard so it works even if no mail client is set up
      if (navigator.clipboard) {
        navigator.clipboard.writeText(c.email).then(() => {
          const originalText = mailLink.textContent;
          mailLink.textContent = "✓ Email copied!";
          setTimeout(() => { mailLink.textContent = originalText; }, 2200);
        }).catch(() => {});
      }
    });
  }
  ...
  if (projectList) {
    projectList.innerHTML = c.projects.map(p => `
      <article class="project${p.image ? ' has-image' : ''}">
        ${p.image ? `
          <div class="project-image-wrap">
            <img class="project-image" src="${escapeHtml(p.image)}" alt="${escapeHtml(p.title)}" loading="lazy" />
            <span class="project-tag project-tag-overlay">${escapeHtml(p.tag)}</span>
          </div>
